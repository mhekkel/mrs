...
private:
	...
	struct M6BufferEntryIterator
	{
		M6BufferEntryIterator(M6File &inFile, int64 inOffset, uint32 inCount,
			...
			: mBits(inFile, inOffset, kM6LargeBitBufferSize), mCount(inCount), mFirstDoc(inFirstDoc), mIDLIxMap(inIDLIxMap), mTerm(1), mDoc(inFirstDoc) {}
...
M6FullTextIx::M6FullTextIx(const fs::path &inDBDirectory, const string &inName)
	: mDocWordLocation(1), mDbDirectory(inDBDirectory), mEntryBuffer(mDbDirectory / inName, eReadWrite), mEntryRun(nullptr), mEntryRunThread(boost::bind(&M6FullTextIx::FlushEntryRuns, this)), mEntryCount(0)
{
...
M6BasicValueIx::M6BasicValueIx(const fs::path &inDBDirectory, const string &inName, M6MultiBasicIndex *inIndex)
	: mEntryBufferFile(inDBDirectory / (inName + ".tmp")), mEntryBuffer(mEntryBufferFile, eReadWrite), mIndex(inIndex)
{
}
...
M6ValueIx<T>::M6ValueIx(const fs::path &inDBDirectory, const string &inName, M6MultiBasicIndex *inIndex)
	: M6BasicValueIx(inDBDirectory, inName, inIndex), mEntryRun(new M6EntryRun), mRunThread(boost::bind(&M6ValueIx::FlushEntryRuns, this))
{
}
template <typename T>
...
M6BasicIx::M6BasicIx(M6FullTextIx &inFullTextIndex, M6Lexicon &inLexicon,
					 const string &inName, uint8 inIndexNr)
	: mIndexNr(inIndexNr), mFullTextIndex(inFullTextIndex), mLexicon(inLexicon), mLastDoc(0), mDocCount(0), mDbDocCount(0), mFlushThread(boost::bind(&M6BasicIx::FlushThread, this))
{
}
...
M6StringIx::M6StringIx(M6FullTextIx &inFullTextIndex, M6Lexicon &inLexicon,
					   const string &inName, uint8 inIndexNr, M6BasicIndexPtr inIndex)
	: M6BasicIx(inFullTextIndex, inLexicon, inName, inIndexNr), mIndex(dynamic_cast<M6MultiBasicIndex *>(inIndex.get()))
{
	assert(mIndex);
...
M6TextIx::M6TextIx(M6FullTextIx &inFullTextIndex, M6Lexicon &inLexicon,
				   const string &inName, uint8 inIndexNr, M6BasicIndexPtr inIndex)
	: M6BasicIx(inFullTextIndex, inLexicon, inName, inIndexNr), mIDLFile(nullptr), mIDLBits(nullptr), mIDLOffset(0), mIndex(dynamic_cast<M6MultiIDLBasicIndex *>(inIndex.get()))
{
	assert(mIndex);
	mIndex->SetAutoCommit(false);
	mIndex->SetBatchMode(inLexicon);
...
M6WeightedWordIx::M6WeightedWordIx(M6FullTextIx &inFullTextIndex, M6Lexicon &inLexicon,
								   const string &inName, uint8 inIndexNr, M6BasicIndexPtr inIndex)
	: M6BasicIx(inFullTextIndex, inLexicon, inName, inIndexNr), mIndex(dynamic_cast<M6WeightedBasicIndex *>(inIndex.get()))
{
	assert(mIndex);
	mIndex->SetAutoCommit(false);
	mIndex->SetBatchMode(inLexicon);
...
M6BatchIndexProcessor::M6BatchIndexProcessor(M6DatabankImpl &inDatabank, M6Lexicon &inLexicon)
	: mFullTextIndex(inDatabank.GetDbDirectory(), "full-text.tmp"), mDatabank(inDatabank), mLexicon(inLexicon)
{
}
									   ...
									   M6DataType inDataType, const string &inValue, bool inUnique, uint32 inDocNr)
{
	if (inUnique)
	{
		...
		{
		case eM6StringData:
			index = mDatabank.CreateIndex(inIndexName, eM6CharIndex);
			break;
		case eM6NumberData:
			index = mDatabank.CreateIndex(inIndexName, eM6NumberIndex);
			break;
		case eM6FloatData:
			index = mDatabank.CreateIndex(inIndexName, eM6FloatIndex);
			break;
			//            case eM6DateData:    index = mDatabank.CreateIndex(inIndexName, eM6DateIndex); break;
		default:
			THROW(("Runtime error, unexpected index type"));
		}
...
void M6BatchIndexProcessor::Finish(uint32 inDocCount)
{
	...
	M6BasicIxDesc allDesc = {new M6WeightedWordIx(mFullTextIndex, mLexicon, "full-text",
							 ...
							 "full-text", eM6CharWeightedIndex};
...
M6DatabankImpl::M6DatabankImpl(M6Databank &inDatabank, const fs::path &inPath, MOpenMode inMode)
	: mDatabank(inDatabank), mDbDirectory(inPath), mFastaFile(nullptr), mMode(inMode), mStore(nullptr), mDictionary(nullptr), mBatch(nullptr)
{
...
M6DatabankImpl::M6DatabankImpl(M6Databank &inDatabank, const string &inDatabankID,
	...
	: mDatabank(inDatabank), mID(inDatabankID), mDbDirectory(inPath), mVersion(inVersion), mFastaFile(nullptr), mMode(eReadWrite), mStore(nullptr), mDictionary(nullptr), mBatch(nullptr)
{
	if (fs::exists(inPath))
...
M6BasicIndexPtr M6DatabankImpl::CreateIndex(const string &inName, M6IndexType inType)
{
	...
	if (result == nullptr)
	{
		...
		{
		case eM6CharIndex:
			result.reset(new M6SimpleIndex(path, mMode));
			break;
			//            case eM6DateIndex:		result.reset(new M6SimpleIndex(path, mMode)); break;
		case eM6NumberIndex:
			result.reset(new M6NumberIndex(path, mMode));
			break;
		case eM6FloatIndex:
			result.reset(new M6FloatIndex(path, mMode));
			break;
		case eM6CharMultiIndex:
			result.reset(new M6SimpleMultiIndex(path, mMode));
			break;
			//            case eM6DateIndex:		result.reset(new M6SimpleMultiIndex(path, mMode)); break;
		case eM6NumberMultiIndex:
			result.reset(new M6NumberMultiIndex(path, mMode));
			break;
		case eM6FloatMultiIndex:
			result.reset(new M6FloatMultiIndex(path, mMode));
			break;
		case eM6CharMultiIDLIndex:
			result.reset(new M6SimpleIDLMultiIndex(path, mMode));
			break;
		case eM6CharWeightedIndex:
			result.reset(new M6SimpleWeightedIndex(path, mMode));
			break;
		case eM6LinkIndex:
			result.reset(new M6SimpleMultiIndex(path, mMode));
			break;
...
public:
	M6Accumulator(uint32 inDocCount)
		: mItems(reinterpret_cast<M6Item *>(calloc(sizeof(M6Item), inDocCount))), mFirst(nullptr), mDocCount(inDocCount), mHitCount(0) {}
								 ...
								 M6Iterator *inFilter, bool inAllTermsRequired, uint32 inReportLimit)
{
	...
	auto compare = [](const pair<uint32, float> &a, const pair<uint32, float> &b) -> bool { return a.second > b.second; };
